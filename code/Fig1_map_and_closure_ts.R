...
load(file.path(landingsdir, "CA_dungeness_landings_data.Rdata"))
blocks <- sf::st_read(dsn=gisdir, layer="CA_fishing_blocks")
blocks_wgs84 <- sf::st_transform(blocks, "+init=epsg:4326")
...
port_tl <- cdfw_port %>% 
  ...
  filter(!is.nan(tl_lb_avg))


# Plot landings
g1 <- ggplot() +
  # Plot data
  geom_sf(data=blocks_wgs84, fill=NA, col="grey30", size=0.1) +
  geom_sf(data=usa, fill="grey85", col="white", size=0.2) +
  geom_sf(data=mexico, fill="grey85", col="white", size=0.2) +
  geom_point(data=port_tl, mapping=aes(x=long_dd, y=lat_dd, size=tl_lb_avg/1e6)) +
  # Title and labels
  ggtitle("A. Landings by port and block") + 
  annotate("text", x=port_tl$long_dd+0.3, y=port_tl$lat_dd, label=port_tl$port, hjust=0, size=1.2) +
  # Crops
  coord_sf(xlim = c(-125, -116), ylim = c(32, 42)) +
  # Legends
  scale_size_continuous(name="Mean landings\n(millions of lbs)", range=c(0.1, 2.5)) +
  # Add north/south line
  geom_hline(yintercept=38.77, size=0.2) + 
  annotate("text", x=-115.7, y=39, label="North", hjust=1, size=1.8) +
  annotate("text", x=-115.7, y=38.55, label="Central", hjust=1, size=1.8) +
  # Little things
  labs(x="", y="") +
  theme_bw() + my_theme
g1
