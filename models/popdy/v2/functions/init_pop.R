...
init_pop <- function(b0_mt, step_key, blocks, nwhales, nweeks){
  
  # Population container
  pop_df <- expand_grid(step_key,
                        select(blocks, block_region, block_ramp, block_dzone, block_id, block_sqkm, pbiomass), 
    ...
    select(year, season, date, week, 
           block_region, block_ramp, block_dzone, block_id, block_sqkm, fishing_yn, closure,
    ...
    select(-pbiomass) %>% 
    # Reduce to number of weeks being simulated
    filter(week<=nweeks)
